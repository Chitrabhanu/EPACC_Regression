...
for file_num in range(last_file_num-1, num_files):
    ...
    for fold_num, fold in enumerate(k_folds):
        ...
        for epoch in range(last_epoch_num, num_epochs):
            ...
            #print('')
            metrics_df = pd.DataFrame([[file_num+1, fold_num+1, epoch, avg_train_mae]],
                                      columns = ['split', 'fold', 'epoch', 'MAE'])
            ...
            metrics_df = pd.DataFrame([[avg_val_mae]],
                                      columns = ['MAE'])
            ...
            if epoch==num_epochs - 1:
                ...
                metrics_df.to_csv('pretrained_weights_3LBNSE_abp_cvp_3sol_reg_v3.csv', index=False, mode='a')
            
            if epoch==20:
                metrics_df['Epoch'] = [epoch]*metrics_df.shape[0]
                metrics_df['Fold'] = [fold_num+1]*metrics_df.shape[0]
                metrics_df['File'] = [file_num+1]*metrics_df.shape[0]
                metrics_df.to_csv('pretrained_weights_3LBNSE_abp_cvp_3sol_reg_v3_epoch20.csv', index=False, mode='a')
                
            if epoch==35:
                metrics_df['Epoch'] = [epoch]*metrics_df.shape[0]
                metrics_df['Fold'] = [fold_num+1]*metrics_df.shape[0]
                metrics_df['File'] = [file_num+1]*metrics_df.shape[0]
                metrics_df.to_csv('pretrained_weights_3LBNSE_abp_cvp_3sol_reg_v3_epoch35.csv', index=False, mode='a')
            ...
            if epoch==50:
                ...
                metrics_df['Fold'] = [fold_num+1]*metrics_df.shape[0]
                metrics_df['File'] = [file_num+1]*metrics_df.shape[0]
                metrics_df.to_csv('pretrained_weights_3LBNSE_abp_cvp_3sol_reg_v3_epoch50.csv', index=False, mode='a')

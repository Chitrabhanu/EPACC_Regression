...
selected_model = '1DCNN_3_layer_SE_BN'
...
logging_progress_path = 'Results/EPACC/Classifier/'
model_path = 'Models/EPACC/Classifier/'
plots_path = logging_progress_path + 'plots/pretrain/abp_rand_3sol/'+ selected_model + '/'
...
training_log_file = 'EPACC_'+selected_model+'_model_training_metrics_ptabprand.csv'
...
validation_log_file = 'EPACC_'+selected_model+'_model_validation_metrics_ptabprand.csv'
...
final_fold_performance = 'EPACC_'+selected_model+'_model_avg_fold_validation_performance_ptabprand.csv'
...
for file_num in range(last_file_num-1, num_files):
    ...
    fold_performance = []
    for fold_num, fold in enumerate(k_folds[:3]):


        print('Fold ' + str(fold_num+1))
        train_df_X = train_df[waveform_column_list]
        train_df_Y = train_df[['binary_label']]
        ...
        train_Ytensor_t_fold = torch.LongTensor(train_df_Y_t_fold['binary_label'].tolist())
        ...
        train_Ytensor_v_fold = torch.LongTensor(train_df_Y_v_fold['binary_label'].tolist())
        ...
        if pretrain:
            if not pretrain_loaded:
                ...
                model.load_state_dict(torch.load('abp_rand_pretrainer_state_dict.pth'))
        ...
        for epoch in range(last_epoch_num, num_epochs):
            ...
            for batch_data, batch_targets in train_dataloader:
                ...
                # Collect predicted scores and true labels for metrics calculation
                predicted_scores.extend(F.softmax(output, dim=1)[:, 1].tolist())
            ...
            # Calculate Accuracy and AUROC for the epoch
            accuracy = correct / total
            auroc = roc_auc_score(true_labels, predicted_scores)

            # Calculate Precision, Recall, Specificity, and AUPRC
            precision, recall, thresholds = precision_recall_curve(true_labels, predicted_scores)
            tn, fp, fn, tp = confusion_matrix(true_labels, [1 if score >= 0.5 else 0 for score in predicted_scores]).ravel()
            precision50pct = tp / (tp + fp)
            recall50pct = tp / (tp + fn)
            ...
            #print('')
            metrics_df = pd.DataFrame([[accuracy, auroc, precision50pct, recall50pct, specificity, auprc]],
                                      columns = ['Accuracy', 'AUROC', 'Precision', 'Recall', 'Specificity', 'AUPRC'])
            ...
            with torch.no_grad():
                ...
                val_auroc = roc_auc_score(true_labels, predicted_scores)

                # Calculate Precision, Recall, Specificity, and AUPRC
                precision, recall, thresholds = precision_recall_curve(true_labels, predicted_scores)
                tn, fp, fn, tp = confusion_matrix(true_labels, [1 if score >= 0.5 else 0 for score in predicted_scores]).ravel()
                val_precision50pct = tp / (tp + fp)
                val_recall50pct = tp / (tp + fn)
                val_specificity = tn / (tn + fp)
                val_auprc = auc(recall, precision)

            metrics_df = pd.DataFrame([[val_accuracy, val_auroc, val_precision50pct, val_recall50pct, val_specificity, val_auprc]],
                                      columns = ['Accuracy', 'AUROC', 'Precision', 'Recall', 'Specificity', 'AUPRC'])
            ...
            val_aurocs.append(round(val_auroc, 2))
            ...
            if epoch==num_epochs - 1:
                ...
                metrics_df.to_csv('pretrained_weights_3LBNSE_abprand_3sol_v2.csv', index=False, mode='a')
                
            if epoch==35:
                metrics_df['Epoch'] = [epoch]*metrics_df.shape[0]
                metrics_df['Fold'] = [fold_num+1]*metrics_df.shape[0]
                metrics_df['File'] = [file_num+1]*metrics_df.shape[0]
                metrics_df.to_csv('pretrained_weights_3LBNSE_abprand_3sol_v2_epoch35.csv', index=False, mode='a')
                
            if epoch==50:
                metrics_df['Epoch'] = [epoch]*metrics_df.shape[0]
                metrics_df['Fold'] = [fold_num+1]*metrics_df.shape[0]
                metrics_df['File'] = [file_num+1]*metrics_df.shape[0]
                metrics_df.to_csv('pretrained_weights_3LBNSE_abprand_3sol_v2_epoch50.csv', index=False, mode='a')
        ...
        plt.legend()
        plt.show()
        plt.savefig(plots_path + 'Accuracy_split_' + str(file_num+1) + '_fold_' + str(fold_num+1) + '.jpg')
        plt.close()


        plt.plot(aurocs, label = 'Training AUROC')
        ...
        plt.legend()
        plt.show()
        plt.savefig(plots_path + 'AUROC_split_' + str(file_num+1) + '_fold_' + str(fold_num+1) + '.jpg')
        plt.close()
    ...
    metrics = ['Accuracy', 'AUROC', 'Precision', 'Recall', 'Specificity', 'AUPRC']
